...
impl Student {
	pub fn load_from_roster<P: AsRef<Path>>(path: P) -> Vec<Student> {
		...
		let header_map: HashMap<&str, usize> = headers.iter().enumerate().map(|(i, h)| (h, i)).collect();
	...
	fn from_record(record: &StringRecord, name_index: usize, id_index: usize, sis_login_index: usize) -> Option<Student> {
		if record.len() < 3 {
